...
class CMIAttentionMatrix(nn.Module):
    ...
    def __init__(self, dim_in_q, dim_in_k, msg_dim, bias=True, scale=True):
        ...
        self.linear = nn.Linear(in_features=1024, out_features=16)   # *** For Carracing - Addition
    ...
    def modulate(self, context, k): # *** Addition
        # mod_out = F.relu6(torch.square(k) + torch.multiply(2,k) + torch.multiply(context, (1 + torch.abs(k))))
        mod_out = F.relu6(torch.square(k) + 2* k + torch.matmul((1 + torch.abs(k)), context)) # For Carracing
        # mod_out = ((k * (1 + torch.exp(q * k))) / 2)
        # mod_out = (k + (k*q))
        # mod_out = (k*(1+torch.tanh(k*q)))
        # mod_out = (k* (np.power(2, k*q)))
        return mod_out
    ...
    def forward(self, data_q, data_k):
        ...
        if data_q.ndim == data_k.ndim == 2:
            dot = torch.matmul(q, k_mod.T)
        else:
            dot = torch.bmm(q, k_mod.permute(0, 2, 1))
...
class CMISelfAttentionMatrix(CMIAttentionMatrix):
    """CMI Self-attention matrix."""

    def __init__(self, dim_in, msg_dim, bias=True, scale=True):
        super(CMISelfAttentionMatrix, self).__init__(
            dim_in_q=dim_in,
            dim_in_k=dim_in,
            msg_dim=msg_dim,
            bias=bias,
...
class CMIAttentionLayer(nn.Module):
    ...
    def __init__(self, dim_in_q, dim_in_k, dim_in_v, msg_dim, out_dim):
        ...
        self.attention_matrix = CMIAttentionMatrix(
            dim_in_q=dim_in_q,
            dim_in_k=dim_in_k,
            msg_dim=msg_dim,
        )
...
class CMIAttentionNeuronLayer(nn.Module):
    """CMI Permutation invariant layer."""
    ...
    def __init__(self,
                 ...
                 bias=True,
                 scale=True):
        super(CMIAttentionNeuronLayer, self).__init__()
        ...
        self.attention = CMISelfAttentionMatrix(
