...
@gin.configurable
class AcrobotTask(RLTask):
    """Acrobot task with optional observation shuffling and noise."""
    ...
    def __init__(self,
                 ...
                 v=True,
                 num_noise_channels=0):
        super(AcrobotTask, self).__init__(v=v)
        self.shuffle_on_reset = shuffle_on_reset
        self.render = render
    ...
    def seed(self, seed=None):
        self.rnd = np.random.RandomState(seed=seed)
        return super(AcrobotTask, self).seed(seed)
    ...
    def reset_for_rollout(self):
        ...
        if self.verbose:
            print('perm_ix: {}'.format(self.perm_ix))
        return super(AcrobotTask, self).reset_for_rollout()
    ...
    def show_gui(self):
        if self.render:
            time.sleep(0.01)
            return super(AcrobotTask, self).show_gui()
