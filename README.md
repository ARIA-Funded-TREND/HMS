...
Single-layer Co4 vs. Transformer \cite{dosovitskiy2020image} trained on Mini-ImageNet for 30 epochs. Visualization of the complete attention distribution over \textit{N} input tokens, showing more scattered attention and less selective localization in the standard attention-only Vision Transformer (ViT) (Dosovitskiy, 2020). In contrast, Co4 shows more centered, context-aware activation patterns. Notably, no top-k pooling was applied.
